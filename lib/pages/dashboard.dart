...
class _DashboardState extends State<Dashboard> {
  ...
  Future<void> _fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status']) {
        setState(() {
          _monthlySales =
              data['sales'] != null ? data['sales']['mnt_sales'] : null;
          _monthlyTarget =
              data['target'] != null ? data['target']['tar_qty'] : null;
          _isLoading = false;
        });
      } else {
  ...
  Future<void> _fetchAnnualData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status']) {
        setState(() {
          _annualSales = data['sales'] != null ? data['sales']['sales'] : null;
          _annualTarget =
              data['target'] != null ? data['target']['target'] : null;
          _Loading = false;
        });
      } else {
        // API request failed, handle the error accordingly
                          ...
                          children: [
                            ...
                            GestureDetector(
                              ...
                              child: Row(
                                children: [
                                   Padding(
                                     padding: getPadding(top: 20,right: 10,left: 10),
                                     child: Text("Ledger Download",style: TextStyle(fontSize: 10,color: ColorConstant.blue400),),
                                   ),
                                   SvgPicture.asset(
