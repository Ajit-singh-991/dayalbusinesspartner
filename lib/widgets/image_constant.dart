class ImageConstant {
  // static String imgDayalgrouplogo = 'assets/images/img_dayalgrouplogo.png';

  // static String imgArrowdownBlack900 =
  //     'assets/images/img_arrowdown_black_900.svg';

  static String imgArrowdownGray400 =
      'assets/images/img_arrowdown_gray_400.svg';

  // static String imgLivertonic1 = 'assets/images/img_livertonic1.png';

  static String imgDayalanmolsuper = 'assets/images/img_dayalanmolsuper.png';

  // static String imgBatissa1 = 'assets/images/img_batissa1.png';

  // static String imgVector = 'assets/images/img_vector.svg';

  // static String imgArrowleft = 'assets/images/img_arrowleft.svg';

  // static String imgEllipse12 = 'assets/images/img_ellipse1_2.png';

  // static String imgArrowdown = 'assets/images/img_arrowdown.svg';

  // static String imgVectorRed700 = 'assets/images/img_vector_red_700.svg';

  // static String imgCallRed700 = 'assets/images/img_call_red_700.svg';

  // static String imgClose = 'assets/images/img_close.svg';

  // static String imgArrowup = 'assets/images/img_arrowup.svg';

  static String imgVectorGray600 = 'assets/images/img_vector_gray_600.svg';

  // static String imgVectorWhiteA700 = 'assets/images/img_vector_white_a700.svg';

  // static String imgHomeGray600 = 'assets/images/img_home_gray_600.svg';

  // static String imgGroup81 = 'assets/images/img_group81.svg';

  // static String imgArrowright = 'assets/images/img_arrowright.svg';

  // static String imgDayaldairyplus102x76 =
  //     'assets/images/img_dayaldairyplus_102x76.png';

  // static String imgUnsplashwnolnjo7ts884x84 =
  //     'assets/images/img_unsplashwnolnjo7ts8_84x84.png';

  static String imgArrowdownBlueGray900 =
      'assets/images/img_arrowdown_blue_gray_900.svg';

  // static String imgEllipse11039x390 = 'assets/images/img_ellipse1_1039x390.png';

  // static String imgEllipse11 = 'assets/images/img_ellipse1_1.png';

  // static String imgUnsplashifgrcqhznqg =
  //     'assets/images/img_unsplashifgrcqhznqg.png';

  // static String imgGroup891 = 'assets/images/img_group891.png';

  static String imgDayalsuperaction = 'assets/images/img_dayalsuperaction.png';

  // static String imgUnsplashwnolnjo7ts8 =
  //     'assets/images/img_unsplashwnolnjo7ts8.png';

  static String imgPlus = 'assets/images/img_plus.svg';

  // static String imgDayalmilktonic = 'assets/images/img_dayalmilktonic.png';

  // static String imgDayaltastymineral =
  //     'assets/images/img_dayaltastymineral.png';

  // static String imgCall = 'assets/images/img_call.svg';

  // static String imgDayalgrouplogo134x110 =
  //     'assets/images/img_dayalgrouplogo_134x110.png';

  // static String imgEllipse1 = 'assets/images/img_ellipse1.png';

  // static String imgCheckmark = 'assets/images/img_checkmark.svg';

  // static String imgRectangle5 = 'assets/images/img_rectangle5.png';

  // static String imgCart = 'assets/images/img_cart.svg';

  // static String imgTicket = 'assets/images/img_ticket.svg';

  // static String imgUnsplashifgrcqhznqg45x46 =
  //     'assets/images/img_unsplashifgrcqhznqg_45x46.png';

  // static String imgArrowrightBlack900 =
  //     'assets/images/img_arrowright_black_900.svg';

  // static String imgDayalmilktonic1 = 'assets/images/img_dayalmilktonic1.png';

  // static String imgComputer = 'assets/images/img_computer.svg';

  // static String imgCartRed700 = 'assets/images/img_cart_red_700.svg';

  // static String imgAppbanner21 = 'assets/images/img_appbanner21.png';

  static String imgDayalshakti2 = 'assets/images/img_dayalshakti2.png';

  // static String imgDayaldairyplus = 'assets/images/img_dayaldairyplus.png';

  // static String imgHome = 'assets/images/img_home.svg';

  // static String imgArrowleftWhiteA700 =
  //     'assets/images/img_arrowleft_white_a700.svg';

  // static String imgDayalsangampashu = 'assets/images/img_dayalsangampashu.png';

  // static String imageNotFound = 'assets/images/image_not_found.png';
}
